...
const CropPrediction = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('/datasets/crop_recommendation.csv');
      ...
      rows.forEach(row => {
        ...
        if (cols.length === 8) {
          const [n, p, k, temp, hum, ph, rain, label] = cols;
          
          // Euclidean distance weighting ignoring rainfall since user doesn't input it
  ...
  return (
    <div className="prediction-container animate-fade-in">
      <div className="header-text">
        <h1 className="text-gradient">{t('intelligentCropPredictor')}</h1>
        <p>{t('cropPredictorDesc')}</p>
      </div>
        ...
        <div className="form-card glass-panel stagger-1">
          <form onSubmit={handleSubmit} className="prediction-form">
            ...
            <button type="submit" className="btn-primary" disabled={loading}>
              {loading ? t('analyzing') : <>{t('predictCrop')} <Sprout size={18} /></>}
            </button>
          </form>
        </div>
